...
'    <purpose>
      <para>
        This dataset was developed to illustrate how ecological monitoring data from the National Ecological Observatory Network (NEON) can be structured for publication to GBIF using the Darwin Core standard and the Humboldt Extension for Ecological Inventories. The workflow demonstrates how complex survey data can be organized into a Darwin Core Archive that supports reuse, integration, and ecological analysis.
      </para>
    </purpose>

    <introduction>
      <para>
        This example has been created alongside the GBIF Guide for publishing biological survey and monitoring data to GBIF 
        (<ulink url="https://docs.gbif.org/guide-publishing-survey-data/en/"><citetitle>GBIF survey data guide</citetitle></ulink>) 
        to illustrate how data from the National Ecological Observatory Network (NEON) can be expressed using Darwin Core and the Humboldt Extension for Ecological Inventories 
    ...
    <gettingStarted>
      ...
      <para>
        Code used to transform the original NEON data into this Darwin Core Archive, including all data processing and mapping steps, is available at 
        <ulink url="https://sunray1.github.io/NEONTickstoHumboldt/"><citetitle>NEON Ticks to Humboldt workflow</citetitle></ulink>. 
        This resource provides a reproducible workflow for constructing the archive and may be used as a reference for applying the Humboldt Extension to similar ecological datasets.
      </para>
    </gettingStarted>\n'
)
